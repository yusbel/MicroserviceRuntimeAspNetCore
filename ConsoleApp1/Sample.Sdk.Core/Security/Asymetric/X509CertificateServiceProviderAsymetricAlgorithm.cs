...
namespace Sample.Sdk.Core.Security.Asymetric
{
    /// <summary>
    ...
    public class X509CertificateServiceProviderAsymetricAlgorithm : IAsymetricCryptoProvider
    {
        ...
        public async Task<(bool wasValid, EncryptionDecryptionFail reason)>
            VerifySignature(
                byte[] hashValue,
                byte[] baseSignature,
                HostTypeOptions keyVaultOptionsType,
                string certificateName,
                CancellationToken token)
        {
        ...
        private async Task<X509Certificate2> GetCertificate(HostTypeOptions keyVaultOptionsType, string certificateName, CancellationToken token)
        {
            X509Certificate2 certificate = null;
