...
using Sample.Sdk.Http.Interfaces;
using Sample.Sdk.Http.Request;
using Sample.Sdk.Interface.Enums;
using Sample.Sdk.Security;
using Sample.Sdk.Security.Providers.Asymetric.Interfaces;
using System.Text;

namespace Sample.Sdk.Http
{
    /// <summary>
        ...
        public async Task<HttpRequestMessage> Decrypt(HttpRequestMessage request, CancellationToken token)
        {
            ...
            if (request.Content != null)
            {
                (bool wasDecrypted, byte[]? data, EncryptionDecryptionFail reason) plainContent =
                    await _cryptoProvider.Decrypt(await request.Content.ReadAsByteArrayAsync(), Enums.HostTypeOptions.Runtime, "", token);
                ...
                request.Headers.Where(h => h.Key.StartsWith(EncryptedHeaderPrefix) && h.Value.Any(hv => hv.Length > 0))
                    ...
                    .ForEach(async header =>
                    {
                        ...
                        (bool wasDecrypt, byte[]? data, EncryptionDecryptionFail reason) plainStr =
                            await _cryptoProvider.Decrypt(Encoding.UTF8.GetBytes(encryptedStr), Enums.HostTypeOptions.Runtime, "", token);
