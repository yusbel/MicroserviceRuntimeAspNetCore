...
namespace Sample.Sdk.Security.Providers.Protocol
{
    public class PointToPointSessionRoot
    {
            ...
            , CancellationToken token)
        {
            ...
            {
                e.LogCriticalException(_logger, "An error ocurred when decrypting with private key. The certificate does not contin a private key");
                return (false, default, default);
            }
        ...
        protected (bool wasEncrypted, byte[]? data) EncryptWithPublicKey(byte[] cert, byte[] plainText, CancellationToken token)
        {
            ...
            {
                e.LogCriticalException(_logger, "An error ocurred when encrypting with public key");
            ...
            {
                e.LogCriticalException(_logger, "Error ocurred when encrypting data");
                return (false, default);
            }
        }
            ...
            , CancellationToken token)
        {
            ...
            {
                e.LogCriticalException(_logger, "Getting ccertificate fail");
                return (false, default);
            }
        }
            ...
            , string externalWellknownEndpoint)
        {
            ...
            {
                e.LogCriticalException(_logger, "Fail creating session with external service");
                return (false, default);
            }
        }
    }
