...
* Each microservice would use a unique service principal from Azure AD to access azure native service. The runtime uses a unique service principal from Azure AD to access the runtime services. These allows to segregate the permission required by the microservice vs the runtime of the microservices.
